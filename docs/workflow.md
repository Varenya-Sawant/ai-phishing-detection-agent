...
An autonomous, self-learning email security pipeline that triages every incoming Gmail message
through a multi-layer decision engine — combining deterministic rule-based filtering, institutional
memory (sender blocklist/allowlist), domain intelligence, and AI-powered forensic analysis — to
classify, label, archive, and learn from phishing threats with minimal human intervention.
...
Hard-coded allowlist check. If the sender domain exactly Is same list as any of the following, the run
**ends immediately** — no further processing or AI cost incurred:

`google.com` · `github.com` · `microsoft.com` · `apple.com` · `amazon.com` · `linkedin.com` ·
`slack.com` · `notion.so` · `stripe.com`
...
Makes a live HTTP GET request to `rdap.org` using the sender's domain. Retrieves domain registration
events. Continues on error (graceful degradation for domains that don't return RDAP data).
...
| Step | Action |
|---|---|
| Step 9 | Send alert email to user with full context |
...
1. Sender authenticity (display name vs address, lookalike domains)
2. Header authentication (SPF, DKIM, DMARC signals)
3. Social engineering (urgency, fear, authority impersonation)
4. Link analysis (anchor vs URL mismatch, obfuscation, suspicious TLDs)
5. Content quality (generic greetings, unnatural phrasing, scam patterns)
6. Brand impersonation
7. Attachment risk (beyond the pre-computed count)
...
| **Conservative bias** | AI prompt defaults to more conservative verdict when signals
